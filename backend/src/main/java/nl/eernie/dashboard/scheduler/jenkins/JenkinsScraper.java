	...
	private void scrapJob(String jobName, Job job, JenkinsServer jenkinsServer)
	{
		try
		{
			...
			else
			{
				...
				List<LinkedHashMap<String, String>> result = stream.filter((LinkedHashMap<String, String> o) -> o.containsKey("_class") && o.get("_class").equals("hudson.plugins.git.util.BuildData")).collect(Collectors.toList());
